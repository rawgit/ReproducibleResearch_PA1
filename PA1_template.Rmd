...
```
**The mean of number of steps per day is `r sry1mean`, and the median of number of steps is `r sry1median`.**
...
```
**The maximum average number of steps is `r as.integer(sry2mx$avg_steps)` for the interval `r sry2mx$interval`.**


```{r reportmissingvalues}
nnas <- nrow(df[is.na(df),])
```
**The number of missing values in the dataset are `r nnas`.**
...
```
**After flling in missing values, the mean of number of steps per day is `r sry3mean`, and the median of number of steps is `r sry3median`.**
